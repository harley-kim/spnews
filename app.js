const state={items:[],brand:'ALL',query:''};
...
function filtered(){const q=state.query.trim().toLowerCase();return state.items.filter(x=>{const brand=state.brand==='ALL'||x.brand===state.brand;const hay=[x.title,x.summary,x.brand,x.source].join(' ').toLowerCase();return brand&&(!q||hay.includes(q))}).sort((a,b)=>new Date(b.published_at)-new Date(a.published_at))}
function card(x){return `<a class="card" href="${escapeHtml(x.url)}" target="_blank" rel="noopener noreferrer"><div class="card-brand">${brandVisual(x.brand)}</div><div class="card-body"><div class="meta"><span class="badge">NEWS</span><span>${escapeHtml(fmt(x.published_at))}</span></div><h3>${escapeHtml(x.title)}</h3><p class="summary">${escapeHtml(x.summary||'원문에서 자세한 내용을 확인하세요.')}</p></div><div class="card-side"><span class="source">${escapeHtml(x.source||'Original')}</span><span class="arrow">↗</span></div></a>`}
...
async function boot(){try{const res=await fetch(`./data/news.json?v=${Date.now()}`);if(!res.ok)throw new Error('data load failed');const data=await res.json();state.items=(data.items||[]).filter(x=>x.type!=='video');$('#lastUpdated').textContent=data.updated_at?fmt(data.updated_at):'업데이트 정보 없음';renderBrands();render()}catch(e){console.error(e);$('#lastUpdated').textContent='데이터 준비 중';$('#emptyState').hidden=false;$('#emptyState strong').textContent='뉴스 데이터를 불러오지 못했습니다.'}}
